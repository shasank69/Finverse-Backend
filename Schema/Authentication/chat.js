...
function isVideoRequest(message) {
  ...
  return (
    (lower.includes('videos') || lower.includes('youtube') || lower.includes('playlist')) &&
...
If the user asks for YouTube videos, return ONLY 3–5 valid full YouTube links (e.g. https://www.youtube.com/watch?v=XXXXXXXXXXX). No explanations. Do not generate fake URLs. If you don’t know any, say so.
...
`;
    ...
    if (isVideoRequest(message)) {
      ...
      playlists = validUrls.slice(0, MAX_PLAYLISTS).map(url => ({ url }));
